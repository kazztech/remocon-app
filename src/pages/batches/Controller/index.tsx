...
import { Container, ListItem, ListItemText, List, Divider, ListItemSecondaryAction, IconButton } from '@material-ui/core';
...
function Batch(props: BatchProps) {
    ...
    // 赤外線送信
    const { getApi: sendIrApi, status: getIrStatus } = useApi({ timeoutLimitMs: 5000, offsetTimeMs: 500 });

    function irSendApiExec(batchId: number): void {
        sendIrApi(`/api/v1/batches/${batchId}/ir-send`, "GET");
    }

    useEffect(() => {
        props.changePage(40001);
        batchGetApi("/api/v1/batches", "GET");
    }, []);
                    ...
                    <List>
                        ...
                        {batches.map((batch: any, index: any) => (
                            <React.Fragment key={index} >
                                <ListItem alignItems="flex-start" button onClick={() => { irSendApiExec(batch.id) }}>
                                    <ListItemText
                                        primary={batch.name}
                                        secondary={
                                            <React.Fragment>
                                                {batch.widgets.map((widget: any, index: any) => (
                                                    <React.Fragment key={index} >
                                                        {`${index + 1}. ${widget.remocon.name} / ${widget.label.text}`}<br />
                                                    </React.Fragment>
                                                ))}
                                            </React.Fragment>
                                        }
                                    ...
                                    <ListItemSecondaryAction>
                                        <IconButton edge="end" component={Link} to={`/edit/batches/${batch.id}`}>
                                            <InfoOutlinedIcon />
                                        </IconButton>
                                    </ListItemSecondaryAction>
                                </ListItem>
