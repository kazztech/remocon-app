...
import { makeStyles, Container, TextField, Typography, Box, MenuItem, Button, Grid, Dialog, DialogTitle, DialogContent, DialogContentText, DialogActions, CircularProgress, Divider } from '@material-ui/core';
                ...
                <Grid container spacing={1}>
                    {Array.from(Array(100).keys()).map((v, i) => (
                        <Grid key={i} item xs={3}>
                            <Button fullWidth variant="contained" size="large" color={v === selectButton ? "primary" : "default"} onClick={() => {
                                setSelectButton(v)
                            }}>
                                {v}
                            </Button>
...
const IrReadDialog = (props: {
    open: boolean;
    onClose: () => void;
    irPattern: string | null;
    setIrPattern: (position: string | null) => void;
}) => {
    const classes = styles();
    const theme = useTheme();
    const { onClose, open } = props;
    ...
    const irReadApiSending = () => {
        setIsReading(true);
        setScene("connecting");
        Axios.get("http://192.168.3.200:3000/api/v1/ir-read", { timeout: 10000 }).then(response => {
            setTimeout(() => {
    ...
    return (
        ...
        >
            ...
            <DialogContent>
                {!isReading && (
                    <Button onClick={() => { irReadApiSending() }}>赤外線受信開始</Button>
                )}
                <Box style={{ textAlign: "center" }}>
                    ...
                    {isReading && scene === "error" && (
                        <>
                            ...
                            <Button onClick={() => { irReadApiSending() }}>再設定</Button>
                        </>
                    )}
...
const WidgetCreateInput: React.FC<WidgetCreateInputProps> = (props: WidgetCreateInputProps) => {
    const classes = styles();
    ...
    const [inputWidgetLabelText, setInputWidgetLabelText] = useState<string>(
        typeof propsState !== "undefined" ? propsState.inputWidgetLabelText : ""
    );
    const handleInputWidgetLabelText = (e: React.ChangeEvent<HTMLInputElement>) => {
        setInputWidgetLabelText(e.target.value);
    }

    // 入力値.アイコンカラー
    const [inputWidgetIconColor, setInputWidgetIconColor] = useState<string>(
        typeof propsState !== "undefined" ? propsState.inputWidgetIconColor : "black"
    );
    const handleInputWidgetIconColor = (e: React.ChangeEvent<HTMLInputElement>) => {
        setInputWidgetIconColor(e.target.value);
    }

    // 入力値.アイコンスタイル
    const [inputWidgetIconStyle, setInputWidgetIconStyle] = useState<string>(
        typeof propsState !== "undefined" ? propsState.inputWidgetIconStyle : "rectangle"
    );
    const handleInputWidgetIconStyle = (e: React.ChangeEvent<HTMLInputElement>) => {
        setInputWidgetIconStyle(e.target.value);
    }

    // 入力値.レイアウト設定
    const [selectPosition, setSelectPosition] = React.useState<number | null>(
        typeof propsState !== "undefined" ? propsState.selectPosition : null
    );

    // 入力値.赤外線データ
    const [irPattern, setIrPattern] = React.useState<string | null>(
        typeof propsState !== "undefined" ? propsState.irPattern : null
    );

    React.useEffect(() => {
        props.changePage(21501);
    }, []);
        ...
        <>
            <Container className={classes.container}>
                <Grid className={classes.form} container spacing={2}>
                    ...
                    <Grid item xs={12}>
                        <Button variant="contained" fullWidth onClick={() => { setLayoutDialogOpen(true) }}>レイアウト設定：{
                            selectPosition === null ? "未選択" : selectPosition
                        }</Button>
                    </Grid>
                    <Grid item xs={12}>
                        <Button variant="contained" fullWidth onClick={() => { setIrReadDialogOpen(true) }}>赤外線データ：{
                            irPattern === null ? "未設定" : (
                                <>
                                    設定済<CheckBoxIcon />
                                </>
                    ...
                    <Grid item xs={12} className={`${classes.formSubmit}`}>
                        <Button
                            component={Link}
                            to={{
                                pathname: `/edit/remocons/${remoconId}/widgets/${widgetId}/update/confirm`,
                                state: {
            ...
            <LayoutDialog
                ...
                setSelectButton={(position) => { setSelectPosition(position) }}
            />
            <IrReadDialog
                onClose={() => { setIrReadDialogOpen(false) }}
                open={irReadDialogOpen}
                irPattern={irPattern}
                setIrPattern={(position) => { setIrPattern(position) }}
            />
        </>
    );
}
