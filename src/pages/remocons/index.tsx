...
import { makeStyles, Container, Button, Grid, Box, Divider, Theme, Fab } from '@material-ui/core';
...
const Controller: React.FC<ControllerProps> = (props: ControllerProps) => {
    ...
    const { getApi: getSendIrApi, status: sendIrStatus } = useApi({ timeoutLimitMs: 5000, offsetTimeMs: 500 });

    const sendIrApi = (widgetId: number) => {
        getSendIrApi(`http://192.168.3.200:3000/api/v1/widgets/${widgetId}/ir-send`, "GET");
    };
    ...
    const Widgets = (props: ControllerProps) => {
        const sortedWidgets = (() => {
            const maxLength = Math.max.apply(null, remocons[selectRemoconIndex].widgets.map((widget: WidgetType) => (widget.position.y * 4 + widget.position.x)));
            const result = [];
            for (let i = 0; i < maxLength + 1; i++) {
                const r = remocons[selectRemoconIndex].widgets.find(widget => (
